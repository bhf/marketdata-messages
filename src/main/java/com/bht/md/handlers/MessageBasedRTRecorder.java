...
public class MessageBasedRTRecorder extends AbstractSBEDecodingHandler {
    ...
    public void handleSingleSidedQuote(SingleSidedQuoteDecoder msgDecoder, int stream, int session) {
        ...
        perfTracker.handleMsgRT(tid, stream, session, now);
    }
    ...
    @Override
    public void handleGroupedSidedUpdate(GroupedSidedUpdateDecoder msgDecoder, int stream, int session) {
        long now = System.nanoTime();
        int tid = msgDecoder.sbeTemplateId();
        perfTracker.handleMsgRT(tid, stream, session, now);
    }
    ...
    @Override
    public void handleMultiSymbolMassBidsUpdate(MultiSymbolMassBidsUpdateDecoder msgDecoder, int stream, int session) {
        long now = System.nanoTime();
        int tid = msgDecoder.sbeTemplateId();
        perfTracker.handleMsgRT(tid, stream, session, now);
    }
    ...
    @Override
    public void handleMultiSymbolMassAsksUpdate(MultiSymbolMassAsksUpdateDecoder msgDecoder, int stream, int session) {
        long now = System.nanoTime();
        int tid = msgDecoder.sbeTemplateId();
        perfTracker.handleMsgRT(tid, stream, session, now);
    }

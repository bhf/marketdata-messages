...
/**
 * Hacked version of the Agrona ManyToOneRingBuffer which allows publishing
 * of a stream and session ID explictly along with the message
 */
public final class IdentifyingManyToOneRingBuffer implements RingBuffer {

    public static final int MIN_CAPACITY = HEADER_LENGTH;
    private static final int STREAM_SESSION_LENGTH = 8;
    ...
    public IdentifyingManyToOneRingBuffer(final AtomicBuffer buffer) {
    ...
    @Override
    public boolean write(int i, DirectBuffer directBuffer, int i1, int i2) {
        return false;
    }
    ...
    public boolean write(
        ...
        final int recordLength = length + HEADER_LENGTH + STREAM_SESSION_LENGTH;
        ...
        buffer.putInt(streamOffset(recordIndex), streamId);
        buffer.putInt(sessionOffset(recordIndex), sessionId);
        return true;
    }
    ...
    public static int encodedMsgOffset(int recordOffset) {
        return recordOffset + 16;
    }

    public static int streamOffset(int recordOffset) {
        return recordOffset + 8;
    }

    public static int sessionOffset(int recordOffset) {
    ...
    public int read(final StreamSessionMessageHandler handler) {
        return read(handler, Integer.MAX_VALUE);
    }
     ...
     * {@inheritDoc}
     */
    public int read(final StreamSessionMessageHandler handler, final int messageCountLimit) {
        int messagesRead = 0;
        ...
        try {
            while ((bytesRead < maxBlockLength) && (messagesRead < messageCountLimit)) {
                ...
                final int streamId = buffer.getInt(streamOffset(recordIndex));
                final int sessionId = buffer.getInt(sessionOffset(recordIndex));

                handler.onMessage(
                        messageTypeId,
                        buffer,
                        recordIndex + HEADER_LENGTH,
